...
(function(root) {
  ...
  function bakeOne(settings, controlName, ref, colorMap, typoMap) {
    ...
    if (parsed.kind === "colors") {
      ...
      if (c && typeof c.color !== "undefined") {
        settings[controlName] = c.color;
      }
      return;
    }
    ...
    for (var prop in settings) {
      ...
      if (Array.isArray(val)) {
        for (var i = 0; i < val.length; i++) {
          if (val[i] && typeof val[i] === "object" && !Array.isArray(val[i])) {
            bakeSettings(val[i], colorMap, typoMap);
          }
        }
      }
    }
  ...
  function walk(node, colorMap, typoMap) {
    ...
    if (Array.isArray(node.elements)) {
      for (var i = 0; i < node.elements.length; i++) {
        walk(node.elements[i], colorMap, typoMap);
      }
    }
  }
  function jltmaFlattenGlobals(node, colors, typography) {
    var colorMap = indexById(colors);
    var typoMap = indexById(typography);
    walk(node, colorMap, typoMap);
    return node;
  }
