...
class Comments extends Master_Widget
{
	...
	public function jltma_comment_form($args = array(), $post_id = null)
	{
		...
		/* translators: %s is the required field marker (HTML span). */
		...
		$defaults = array(
			...
			'logged_in_as' 			=> '<p class="logged-in-as">' . sprintf(
				/* translators: %1$s is the user profile URL, %2$s is the aria-label, %3$s is the user's name, %4$s is the logout URL. */
			...
			/* translators: %s is a reply-to name. */
...
?>
		...
		<div class="jltma-comment-listing-wrap" id="jltma-comment-listing-wrap">
			...
			if (isset($settings['jltma_comment_total_number']) && $settings['jltma_comment_total_number'] == 'show') {
				...
				if (is_user_logged_in() && Comments_Builder::jltma_comment_elementor_preview_mode()) {
					...
					echo '5 ' . esc_html( $backend_comment_text ) . ' on ' . esc_html( get_the_title() );
					echo "</h3>";
				} else {
					...
					echo esc_html( $comments_number ) . ' on ' . esc_html( get_the_title() );
					echo "</h3>";
				}
			}
	...
	public function jltma_comment_template()
	{
		...
		<div <?php echo $this->get_render_attribute_string('jltma_comments_wrap'); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Elementor get_render_attribute_string() returns safely escaped HTML attributes ?>>
