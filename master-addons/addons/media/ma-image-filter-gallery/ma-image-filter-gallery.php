...
use MasterAddons\Inc\Classes\Utils as JltmaUtils;
 ...
 */
class Filterable_Image_Gallery extends Master_Widget
{
		...
		*/
		...
		$this->add_control(
			...
			[
				...
				'type'      => Controls_Manager::SELECT,
				'options'   => Helper::jltma_animation_options(),
	...
	protected function render()
	{
		...
		// Always add gallery classes (needed for both editor and frontend)
		$gallery_classes = [
			'jltma-image-filter-gallery',
			'jltma-image-filter-gallery-items',
		];

		// Add editor-specific class to use CSS grid instead of isotope
		if (\Elementor\Plugin::$instance->editor->is_edit_mode()) {
			$gallery_classes[] = 'jltma-editor-mode';
		}
					...
					$this->add_render_attribute([
						$gallery_item_key => [
							'class' => [
								...
								JltmaUtils::image_filter_gallery_category_classes($item['gallery_category_name'], $this->get_id()),
							],
						]
					]);
