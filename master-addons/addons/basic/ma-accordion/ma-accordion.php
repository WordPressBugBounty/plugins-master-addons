...
class Advanced_Accordion extends Master_Widget
{
	...
	{
		return 'jltma-icon ' . Config::get_addon_icon('ma-accordion');
	}
	...
	protected function register_controls()
	{
		...
		// Single Accordion Tab Styles - filtered through Free_Addons.php
		$single_tab_styling_controls = apply_filters('master_addons/addons/accordion/single_tab_styling', [
			[
				'control_id' => 'single_tab_title_bg_color_show_pro',
				'args' => [
					'label' => esc_html__('Enable Background Color (Pro)', 'master-addons'),
					...
					'default' => '1',
					'description' => Helper::upgrade_to_pro('Single Tab Styling available on'),
				],
			],
		]);

		foreach ($single_tab_styling_controls as $control) {
			$repeater->add_control($control['control_id'], $control['args']);
		}
		...
		$content_type_options = apply_filters('master_addons/addons/accordion/content_type', [
		...
		$repeater->add_control(
			...
			[
				...
				'default'     => 'content',
				'description' => Helper::upgrade_to_pro('3+ more Content Types on'),
			]
		);
		...
		$saved_widget_options = apply_filters('master_addons/addons/accordion/saved_widget', $this->get_page_template_options('widget'));
		...
		$saved_section_options = apply_filters('master_addons/addons/accordion/saved_section', $this->get_page_template_options('section'));
		...
		$templates_options = apply_filters('master_addons/addons/accordion/templates', $this->get_page_template_options('page'));
		...
		$this->add_control(
			...
			[
				...
				'default' => 'left',
				'selectors' => [
					// '{{WRAPPER}} .jltma-advanced-accordion .jltma-accordion-tab-title' => 'text-align: {{VALUE}} !important;',
		...
		$this->start_controls_section(
			...
			[
				'label' => esc_html__('Toggle Icons', 'master-addons'),
				'tab' => Controls_Manager::TAB_STYLE,
				'condition' => [
					'toggle_icon_show' => 'yes'
				]
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .ma-accordion-icon-align-left .jltma-accordion-tab-title .jltma-accordion-toggle-icon' => 'margin-left: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .ma-accordion-icon-align-right .jltma-accordion-tab-title .jltma-accordion-toggle-icon' => 'margin-right: {{SIZE}}{{UNIT}};',
				],
				'condition' => [
					'toggle_icon_show' => 'yes'
				]
		...
		$this->add_control(
			...
			[
				'label' => __('Icon Colors', 'master-addons'),
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .jltma-advanced-accordion .jltma-accordion-tab-title .jltma-accordion-toggle-icon'
			]
		);
		...
		$this->jltma_help_docs('ma-accordion');
	...
	protected function render()
	{
		...
		$this->add_render_attribute([
			'jltma-accordion-wrap' => [
				'class' => [
					'ma-accordion-wrap',
					'ma-accordion-icon-align-' . esc_attr($toggle_icon_position),
				...
				foreach ($settings['tabs'] as $index => $tab) {
					...
					<div
						...
						<<?php echo esc_html($title_html_tag); ?> <?php echo $this->get_render_attribute_string($tab_title_setting_key);
											...
											// Single tab styling - only apply if Pro control exists
											if (isset($tab['single_tab_title_bg_color_show']) && $tab['single_tab_title_bg_color_show'] == 'yes') { ?>
									style="background-color:<?php echo esc_attr($tab['single_tab_title_bg_color']); ?>;
									color:<?php echo esc_attr($tab['single_title_text_color']); ?>" <?php } ?>>
								...
								if ($settings['toggle_icon_show'] === 'yes' && ($settings['toggle_icon_position'] == "left")) {
									if ($tab['accordion_tab_icon_show'] === 'yes') { ?>
										...
										<?php
											...
											if ($is_new || $migrated) {
												$this->render_icon($tab['accordion_tab_title_icon_collapse'], ['aria-hidden' => 'true', 'class' => 'jltma-el-accordion-icon-closed']);
											} else { ?>
												<i class="jltma-el-accordion-icon-closed <?php echo esc_attr($tab['icon_collapse']); ?>"
													aria-hidden="true"></i>
											<?php }
											...
											if ($active_is_new || $active_migrated) {
												$this->render_icon($tab['accordion_tab_title_icon'], ['aria-hidden' => 'true', 'class' => 'jltma-el-accordion-icon-opened']);
											} else { ?>
									...
									<?php } else { ?>
										...
										<?php
											if (!empty($settings['toggle_icon']['value'])) {
												$this->render_icon($settings['toggle_icon'], ['aria-hidden' => 'true', 'class' => 'jltma-el-accordion-icon-closed']);
											}

											if (!empty($settings['active_icon']['value'])) {
												$this->render_icon($settings['active_icon'], ['aria-hidden' => 'true', 'class' => 'jltma-el-accordion-icon-opened']);
											}
										?>
										</span>
									<?php }
