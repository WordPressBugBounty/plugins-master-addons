...
class Team_Slider extends Master_Widget
{
	...
	protected function register_controls()
	{
		...
		$carousel_preset_options = apply_filters('master_addons/addons/team_slider/preset', [
		...
		$this->add_control(
			...
			[
				...
				'default'            => '-default',
				'options'            => $carousel_preset_options,
				'frontend_available' => true,
				'description'        => Helper::upgrade_to_pro('3+ more Variations on'),
			]
		);

		// Circle image options - Pro version overrides via filter
		$circle_image_options = apply_filters('master_addons/addons/team_slider/circle_image', [
		...
		$this->add_control(
			...
			[
				...
				'type'        => Controls_Manager::SELECT,
				'default'     => 'circle_01',
				'options'     => $circle_image_options,
				'condition'   => [
					'ma_el_team_carousel_preset' => '-circle'
				],
				'description' => Helper::upgrade_to_pro('4+ more Circle Variations on'),
			]
		);

		// Animation style options - Pro version overrides via filter
		$animation_style_options = apply_filters('master_addons/addons/team_slider/animation_style', [
		...
		$this->add_control(
			...
			[
				...
				'description' => Helper::upgrade_to_pro('More Animated Variations on'),
			]
		);
		...
		<?php if ($team_preset == '-content-drawer') { ?>
			...
			<div <?php echo $this->get_render_attribute_string('ma_el_team_slider_section'); ?>>
				...
				<ul class="gridder">
					...
					if (
						empty($settings['team_carousel_repeater'][0]['ma_el_team_carousel_image'])
						&& empty($settings['team_carousel_repeater'][1]['ma_el_team_carousel_image'])
						&& empty($settings['team_carousel_repeater'][0]['ma_el_team_carousel_image'])
					) {
						$demo_images[] = Helper::jltma_placeholder_images();
					}
							...
							<?php if ($member['ma_el_team_carousel_enable_social_profiles'] == 'yes'): ?>
								<ul class="list-inline jltma-team-member-social">
									...
									<?php if (!empty($member['ma_el_team_carousel_email_link'])): ?>
										<li>
											<a href="mailto:<?php echo esc_attr($member['ma_el_team_carousel_email_link']); ?>"><i
