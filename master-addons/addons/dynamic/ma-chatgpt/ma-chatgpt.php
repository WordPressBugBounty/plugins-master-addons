...
class CHATGPT extends Master_Widget {
	...
	protected function register_controls() {
		...
		if (
			!function_exists('jltma_core_helper') ||
		...
		if ( function_exists( 'lqd_elementor_add_button_controls' ) ) {
	...
	protected function render() {
		...
		<div class="lqd-chatgpt flex flex-col">
			<form <?php $this->print_render_attribute_string( 'lqd_chatgpt_form' ); ?>>
				...
				<?php if ( class_exists( '\LQD_Elementor_Render_Button' ) ) { \LQD_Elementor_Render_Button::get_button( $this, 'ib_', '', 'submit' ); } ?>
				<?php if ( ! empty( $settings['selected_icon']['value'] ) ) { ?>
					<div class="lqd-chatgpt--icon flex items-center justify-center">
						<?php if ( class_exists( '\LQD_Elementor_Helper' ) ) { \LQD_Elementor_Helper::render_icon( $settings['selected_icon'], [ 'aria-hidden' => 'true', 'class' => 'w-1em h-auto align-middle fill-current relative' ] ); } ?>
					</div>
				<?php } ?>
			</form>
