...
class Template_Kit_Cache
{
    ...
    {
        $this->is_pro_enabled = Helper::jltma_premium();
        $upload_dir = wp_upload_dir();
        $basedir = !empty($upload_dir['basedir']) ? $upload_dir['basedir'] : '';
        $this->cache_dir = $basedir . '/master_addons/templates_kits/';
        $this->purchased_dir = $basedir . '/master_addons/purchased_kits/';
    ...
    /**
     * Schedule cache update events for template kits via Background_Task_Manager
     */
    public function schedule_cache_updates()
    {
        Background_Task_Manager::get_instance()->schedule_recurring(
            'jltma_template_kits_cache_update',
            12 * HOUR_IN_SECONDS
        );
    }
    ...
    public function update_template_kits_cache()
    {
        ...
        // Update template kits with retry logic
        try {
            Background_Task_Manager::get_instance()->execute_with_retry(function () {
                $this->get_cached_kits(true);
            }, 3, 'template_kits_cache_sync');
        } catch (\Exception $e) {
            // Failure logged by execute_with_retry
        }
...
Template_Kit_Cache::get_instance();