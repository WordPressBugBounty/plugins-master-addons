...
class CustomJs
{
    ...
    public function jltma_add_section_custom_js_controls($widget, $section_id, $args)
    {
        ...
        // Don't show on page/document settings — that's handled by the document-level hook
        $allowed_types = array('widget', 'section', 'column', 'container');
        if (!in_array($widget->get_type(), $allowed_types)) {
            return;
        }
