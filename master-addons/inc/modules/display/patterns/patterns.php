 ...
 */
if (!class_exists('MasterAddons\Modules\Display\Patterns')) {
class Patterns extends \MasterAddons\Inc\Classes\Extension_Prototype
{
    ...
    {
        // Add patterns section to section/column/container after their background sections
        // and to all widgets after _section_style (common style section)
        $is_layout = (
        ...
        $is_widget = (
            !in_array($element->get_name(), ['section', 'column', 'container']) &&
            '_section_style' === $section_id
        );

        if ($is_layout || $is_widget) {
    ...
    {
        // Only modify JS templates for layout elements (section/column/container).
        // Widgets use server-side PHP rendering; their patterns are handled by before_render().
        if ( ! in_array( $widget->get_name(), [ 'section', 'column', 'container' ], true ) ) {
            return $template;
        }
