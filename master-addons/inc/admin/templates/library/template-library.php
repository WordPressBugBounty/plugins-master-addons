...
class Template_Library {
    ...
    /**
     * Enqueue Template Library JS & CSS on its admin page
     */
    public function enqueue_assets($hook) {
        // Match the screen for this submenu page (handles white-label menu renaming)
        if (strpos($hook, 'jltma-template-library') === false) {
            return;
        }

        // Enqueue registered assets from Assets_Manager
        wp_enqueue_style('jltma-template-library');
        wp_enqueue_style('jltma-page-importer');
        wp_enqueue_script('jltma-template-library');
    ...
    private function fetch_real_templates($tab = 'master_section', $category = '', $search = '', $page = 1, $per_page = 12) {
        ...
        if (!class_exists('MasterAddons\\Inc\\Admin\\Templates\\Includes\\Classes\\Manager')) {
            return [
                'templates' => [],
                'pagination' => [
                    'current_page' => $page,
        ...
        $manager = \MasterAddons\Inc\Admin\Templates\Includes\Classes\Manager::get_instance();
    ...
    private function get_template_preview_url($template_id, $mode) {
        ...
        if($mode === 'kits'){
            return 'https://master-addons.com/template-kits/?kit=' . $template_id;
        }
        return 'https://master-addons.com/template-library/?template=' . $template_id;
        // if($mode === 'kits'){
    ...
    public function jltma_template_kit_open_kit(){
        ...
        // Validate thumbnail URLs - clear any that point to non-existent local files
        $upload_dir = wp_upload_dir();
        foreach ($templates as &$tpl) {
            if (!empty($tpl['thumbnail']) && strpos($tpl['thumbnail'], $upload_dir['baseurl']) === 0) {
                $local_path = str_replace($upload_dir['baseurl'], $upload_dir['basedir'], $tpl['thumbnail']);
            ...
            if ($manifest && isset($manifest['templates'])) {
                ...
                foreach ($manifest['templates'] as $template) {
                    ...
                    if (!empty($screenshot) && strpos($screenshot, 'http') !== 0) {
                        // It's a relative path, verify file exists before building URL
                        $kit_base_dir = $upload_dir['basedir'] . '/master_addons/templates_kits/kits/' . $kit_id . '/';
                        $local_file = $kit_base_dir . ltrim($screenshot, '/');
